                                ...
                                <table class="table">
                                    <tbody>
                                        ...
                                        <tr>
                                            <th>@lang('Link')</th>
                                            <td class="text-start">
                                                <a href="{{ empty(parse_url($order->link, PHP_URL_SCHEME)) ? 'https://' : null }}{{ $order->link }}" target="_blank">{{ $order->link }}
                                                </a>
                                            </td>
                                        </tr>
                                        <tr>
                                        ...
                                        @if ($order->runs && $order->interval)
                                            <tr>
                                                <th>@lang('Runs')</th>
                                                <td class="text-start">{{ $order->runs }} @lang('Times') [<span class="text--success"> @lang('Exists') {{ $order->remain / ($order->quantity / ($order->runs ? $order->runs : 1)) }} @lang('Times')</span>]</td>
                                            </tr>
                                            <tr>
                                                <th>@lang('Intervals')</th>
                                                <td class="text-start">{{ $order->interval }} @lang('Minutes') [ <span class="text--info">@lang('Action taken ') {{ diffForHumans($order->updated_at) }}</span>]</td>
                                            </tr>
                                        @endif
                                        <tr>
                                            <th>@lang('Start Counter') @if ($order->runs && $order->interval)/@lang('Dripfeed Quantity')@endif</th>
                                            <td class="text-start">
                                                @if ($order->status == Status::ORDER_PENDING || $order->status == Status::ORDER_PROCESSING)
                                                    <input class="form-control" name="start_count" type="text" value="{{ $order->start_counter }}" max="{{ $order->quantity }}" required>
                                                @else
                                                    {{ $order->start_counter }}
                                                @endif
                                            </td>
