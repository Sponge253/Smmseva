...
@section('panel')
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="button--group">
                                                <button class="btn btn-sm btn-outline--primary updateApiSetting" data-id="{{ $apiProvider->id }}" data-api_key="{{ $apiProvider->api_key }}" data-short_name="{{ $apiProvider->short_name }}" data-price="{{ $apiProvider->price }}" data-name="{{ $apiProvider->name }}" data-api_url="{{ $apiProvider->api_url }}" type="button"><i class="las la-pen"></i>
                                                    @lang('Edit')</button>

                                                @if ($apiProvider->status == Status::DISABLE)
                                                    <button class="btn btn-sm btn-outline--success confirmationBtn" data-action="{{ route('admin.api.provider.status', $apiProvider->id) }}" data-question="@lang('Are you sure to enable this API?')" type="button">
                                                        <i class="la la-eye"></i> @lang('Enable')
                                                    </button>
                                                @else
                                                    <button class="btn btn-sm btn-outline--danger confirmationBtn" data-action="{{ route('admin.api.provider.status', $apiProvider->id) }}" data-question="@lang('Are you sure to disable this API?')" type="button">
    ...
    {{-- Add & Edit Api  MODAL --}}
    <div class="modal fade" id="addApiModal" role="dialog" aria-hidden="true" tabindex="-1" a>
