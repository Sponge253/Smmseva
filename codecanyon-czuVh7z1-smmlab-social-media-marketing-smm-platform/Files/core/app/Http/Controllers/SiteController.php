...
class SiteController extends Controller {
    ...
    public function blog() {
        $pageTitle    = "Blogs";
        $general      = gs();
        $blogElements = Frontend::where('data_keys', 'blog.element')->where('tempname', $general->active_template)->latest('id')->paginate(getPaginate());
        $sections     = Page::where('tempname', activeTemplate())->where('slug', 'blog')->first();
        return view('Template::blog', compact('pageTitle', 'blogElements', 'sections'));
    }

    public function blogDetails($slug) {
        $blog        = Frontend::where('slug', $slug)->where('tempname', activeTemplateName())->where('data_keys', 'blog.element')->firstOrFail();
        $pageTitle   = 'Blog Detail';
        $recentBlogs = Frontend::where('slug', '!=', $slug)->where('data_keys', 'blog.element')->where('tempname', activeTemplateName())->latest()->take(8)->get();
